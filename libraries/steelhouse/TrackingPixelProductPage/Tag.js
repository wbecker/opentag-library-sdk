...
qubit.qtag.LibraryTag.define("steelhouse.TrackingPixelProductPage", {
    config: {/*DATA*/
	id: 34675,
...
basket_item_count = (function(){
  var temp = 0;
  for (var index = 0; index < this.getValueForToken("basket_item_quantities").length; index++){
    temp += Number(this.getValueForToken("basket_item_quantities")[index]);
  }
  return temp;
}()),
...
basket_skus = (function(){
        var temp_skus = "";
        for (var sku_index = 0; sku_index < this.getValueForToken("basket_skus_list").length; sku_index++){
          temp_skus += this.getValueForToken("basket_skus_list")[sku_index];
          if (sku_index != this.getValueForToken("basket_skus_list").length - 1) temp_skus += ",";
        }
        return temp_skus;
      })(),
basket_quantities = (function(){
        var temp_quants = "";
        for (var quant_index = 0; quant_index < this.getValueForToken("basket_item_quantities").length; quant_index++){
          temp_quants += this.getValueForToken("basket_item_quantities")[quant_index];
          if (quant_index != this.getValueForToken("basket_item_quantities").length - 1) temp_quants += ",";
        }
        return temp_quants;
      })(), 

n="" + this.getValueForToken("merchant_id") + "",
r=shaddslashes("" + this.getValueForToken("product_name") + ""),
i=shaddslashes("" + this.getValueForToken("product_category") + ""),
s=shaddslashes("" + this.getValueForToken("product_brand") + ""),
o="" + this.getValueForToken("product_price") + "",
u="" + this.getValueForToken("product_price_currency") + "",
a="" + this.getValueForToken("product_image_url") + "",
f="" + this.getValueForToken("product_sku") + "",
l="" + this.getValueForToken("product_inventory_count") + "",
c="" + this.getValueForToken("product_availability_ends") + "",
h="" + this.getValueForToken("basket_subtotal") + "",
p=basket_item_count,
d=basket_skus,
v=basket_quantities,
shadditional="" + this.getValueForToken("additional_info") + "",
