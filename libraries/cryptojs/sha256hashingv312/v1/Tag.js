...
*/
		window.CryptoJS = window.CryptoJS || function(h, s) {
				var f = {}, t = f.lib = {}, g = function() {}, j = t.Base = {
					...
					q = t.WordArray = j.extend({
						...
						concat: function(a) {
							...
							this.clamp();
							if (b % 4)
								for (var e = 0; e < a; e++) c[b + e >>> 2] |= (d[e >>> 2] >>> 24 - 8 *
									(e % 4) & 255) << 24 - 8 * ((b + e) % 4);
					...
					v = f.enc = {}, u = v.Hex = {
						...
						stringify: function(a) {
							var c = a.words;
							a = a.sigBytes;
							for (var d = [], b = 0; b < a; b++) d.push(String.fromCharCode(c[b >>>
								2] >>> 24 - 8 * (b % 4) & 255));
							return d.join("")
						},
						parse: function(a) {
							for (var c = a.length, d = [], b = 0; b < c; b++) d[b >>> 2] |= (a.charCodeAt(
								b) & 255) << 24 - 8 * (b % 4);
		...
		(function(h) {
			for (var s = window.CryptoJS, f = s.lib, t = f.WordArray, g = f.Hasher, f = s.algo,
					j = [], q = [], v = function(a) {
			...
			var a = [],
				f = f.SHA256 = g.extend({
					...
					_doProcessBlock: function(c, d) {
						for (var b = this._hash.words, e = b[0], f = b[1], m = b[2], h = b[3],
								p = b[4], j = b[5], k = b[6], l = b[7], n = 0; 64 > n; n++) {
							if (16 > n) a[n] =
								c[d + n] | 0;
							else {
								var r = a[n - 15],
									g = a[n - 2];
								a[n] = ((r << 25 | r >>> 7) ^ (r << 14 | r >>> 18) ^ r >>> 3) + a[n -
									7] + ((g << 15 | g >>> 17) ^ (g << 13 | g >>> 19) ^ g >>> 10) + a[n -
									16]
							}
							r = l + ((p << 26 | p >>> 6) ^ (p << 21 | p >>> 11) ^ (p << 7 | p >>>
								25)) + (p & j ^ ~p & k) + q[n] + a[n];
