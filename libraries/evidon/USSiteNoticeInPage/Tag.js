...
qubit.qtag.LibraryTag.define("evidon.USSiteNoticeInPage", {
    config: {/*DATA*/
	id: 36188,
  ...
  (function() {
    ...
    function waitForLink(){
      ...
      if (lnk){
        ...
        if (this.getValueForToken("class")){
          waitForClass();
        }else{
          waitForID();
        }
    ...
    function waitForClass(){
      var tag = document.querySelectorAll('.' + this.getValueForToken("name") + '')[Number('' + this.getValueForToken("index") + '')];
      if (tag){
        tag.appendChild(lnk);
      }else{
    ...
    function waitForID(){
      var tag = document.getElementById('' + this.getValueForToken("name") + '');
      if (tag){
        tag.appendChild(lnk);
      }else{
