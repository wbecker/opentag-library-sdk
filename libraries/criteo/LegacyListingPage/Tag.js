...
qubit.qtag.LibraryTag.define("criteo.LegacyListingPage", {
    config: {/*DATA*/
	id: 22657,
...
(function () {
  ...
  var product_ids = "";
  for (var i = 0, ii = this.getValueForToken("product_ids").length; i < ii; i += 1) {
	product_ids += "&i" + (i + 1) + "=" + encodeURIComponent(this.getValueForToken("product_ids")[i]);
        if (i === 2) {
          break;
        }
  }
...
_cr_i.src = domain + "/" + this.getValueForToken("call_parameter") + "/display.js?p1=" + escape("v=2&wi=" + this.getValueForToken("wi") + "&pt1=3" + product_ids) + "&t1=sendEvent&resptype=gif&cb=" + Math.floor(Math.random() * 99999999999);
