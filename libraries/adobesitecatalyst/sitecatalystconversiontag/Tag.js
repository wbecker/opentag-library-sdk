...
qubit.opentag.LibraryTag.define("adobesitecatalyst.sitecatalystconversiontag.Tag", {
	config: {
		/*DATA*/
...
(function() {
  var i = 0, ii = this.valueForToken("product_ids").length, productList = [];

  for (; i < ii; i++) {
    var product = [];
    product.push(this.valueForToken("product_names")[i], this.valueForToken("product_ids")[i]);
    product.push(this.valueForToken("quants")[i], this.valueForToken("categories")[i]);
