	...
	script: function() {
		...
		(function() {
			...
			for (var i = 0; i < this.valueForToken("productNames").length; i++) {
				basket = basket + "pr(gr(" + this.valueForToken("productCategories")[i] + ")i(" + this.valueForToken("productIDs")[i] + ")n(" + this.valueForToken("productNames")[i] + ")v(" + this.valueForToken("productPrices")[i] + ")q(" + this.valueForToken("productQuantities")[i] + "))";
			}
