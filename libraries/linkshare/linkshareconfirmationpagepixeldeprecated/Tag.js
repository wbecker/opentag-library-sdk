...
qubit.opentag.LibraryTag.define("linkshare.linkshareconfirmationpagepixeldeprecated.Tag", {
	config: {
		/*DATA*/
...
(function () {
  ...
  for (i = 0, ii =this.valueForToken("q_list").length; i<ii; i++) {
    ...
    amtList.push(parseInt(parseFloat(this.valueForToken("amt_list")[i])*100)*parseInt(this.valueForToken("q_list")[i]));
  }
