...
(function() {
  ...
  var run = function() {
    for (var i = 0, ii = this.getValueForToken("skus").length; i < ii; i++) {
      window.w1x1.scAdd(this.getValueForToken("skus")[i], this.getValueForToken("quantities")[i], this.getValueForToken("totals_novat")[i], this.getValueForToken("totals_vat")[i], this.getValueForToken("prices")[i], this.getValueForToken("voucher"));
    }
    window.w1x1.scSend();
  };

  safeLoad("" + this.getValueForToken("w1x1_function_url") + "", "_w1x1", function() {
    safeLoad("" + this.getValueForToken("w1x1_config_url") + "", "w1x1", function() {
      run();
    });
  });
