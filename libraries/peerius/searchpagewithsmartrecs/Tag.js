...
qubit.qtag.LibraryTag.define("peerius.searchpagewithsmartrecs.Tag", {
	config: {
		/*DATA*/
...
var PeeriusCallbacks = {
  track: {
    type: "searchresults",
    lang: "" + this.getValueForToken("peerius_language") + "",
    searchResults: {
      term: "" + this.getValueForToken("search_query") + "",
      results: []
    }
  },
...
var ii = this.getValueForToken("search_listing_ids").length;
for (var i = 0; i < ii; i++) {
  PeeriusCallbacks.track.searchResults.results.push({
    refCode: this.getValueForToken("search_listing_ids")[i]
  });
}
		/*~PRE*/
	},
